...
> Redis, Kafka, 분산락 없이도 **실시간 API와 월간 정산 API가 공존하는 고성능 시스템을 설계할 수 있는가**를 검증하고자 했습니다.




## 🎯 **한 줄 요약**
**"Redis 없이도 월 100만 건 월간 정산을 1.6분에 처리하면서, 실시간 서비스 장애 0건을 달성한 실험형 백엔드 구조"**
...
* **실시간 API와의 동시성 보장:** 정산배치 실행 중에도 등급/쿠폰과 관련된 API 요청이 안정적으로 처리되도록 보장
...
# 제 4부 : 최종 해결책 – 로그 테이블 기반 정합성 확보 (Log-Table Architecture)
...
* 과거 질문: “어떻게 락을 최소화할 것인가?” → **잘못된 방향**
* 새로운 질문: “정산과 실시간 API가 **동일한 시점을 바라보도록** 만들 수 있는가?”
* 정합성 문제의 본질은 **동시 수정이 아니라 시점 불일치**였다.

---

## 4.2 설계 개요: 정산 배치와 로그 테이블 기반 정합성 확보

### [대안 1] ~~MVCC 장기 스냅샷~~ (기각)

* **방식:** REPEATABLE READ로 긴 트랜잭션 유지
* **장점:** 단순 구현
* **단점:** 언두/리두 로그 누적, 장애 시 복구 어려움

### [대안 2] ~~테이블 복사~~ (기각)

* **방식:** INSERT ... SELECT로 테이블 통째 복사
* **장점:** 읽기와 쓰기 완전 분리
* **단점:** 대량 데이터 시 부하 큼, 복사 중 에러 시 기준 시점 깨짐

### [대안 3] 전통 로그 테이블 (범용성,기각)

* **방식:** 모든 변경 내역 기록, 정산 시 `MAX(timestamp)`로 최신값 추출
* **장점:** 과거 시점 복원 가능, 범용성 높음
* **단점:** 대규모 데이터에서 그룹화·윈도우 함수·정렬 연산으로 **쿼리 비용 높음**

### [최종 선택] 경량 로그 테이블 + 결과 테이블 (프로젝트 적용)

* **개요:**

  * 원본 테이블은 항상 최신 유지
  * 정산 실행 중 변경 발생 시 최초 변경만 로그 테이블에 기록
  * 정산 조회 시: 로그 있으면 로그, 없으면 원본
  * 산출/적용 단계 분리로 긴 트랜잭션 제거 + 멱등 복구 지원
* **장점:**

  * 실시간 API는 최신 원본 유지 → 실시간성 보장
  * 정산은 로그+원본 조합 → 정합성 보장
  * JOIN + 조건 분기로 충분, `MAX(timestamp)` 불필요 → **쿼리 비용 절감**
* **한계:**

  * 모든 과거 시점 복원 불가 (범용성 낮음)
  * 성능·정합성에 최적화된 실험적 구조
...
### 초기: 정산 기준 시점(T0) 확정 + 즉시 쓰기

* **방식:** 정산 시작 시점(T0)만 확정하고, 계산 직후 `badges` UPDATE
* **장점:** 단순
* **문제점:** 긴 트랜잭션, 락 경합, 중단 시 복구 불가

### 발전 1: 경량 로그 테이블 도입

* **방식:** 정산 중 변경 발생 시 최초 변경 이전 상태를 `badge_log`에 기록
* **장점:** API 차단 없이 T0 정합성 보장
* **한계:** 여전히 긴 트랜잭션 존재, 부분 복구 어려움

### 발전 2: 산출과 적용 분리

* **방식:**

  * **산출 단계:** T0 기준 계산 → 결과를 `PENDING` 상태로 저장
  * **적용 단계:** 단일 리더 WAS가 짧은 Tx로 `APPLIED` 처리
* **효과:** 긴 트랜잭션 제거, 데드락 제거, 멱등 복구 가능
...
* 테이블: `badge_log`
* 규칙: `(user_id, badge_id, period_key)` 최초 변경 1회 기록
* 인덱스: `user_id`, `badge_id`, `created_at`

### 결과 테이블

* `badge_results`, `level_results`
* `execution_id`, `status(PENDING|APPLIED|FAILED)`
* 유니크 키: `(execution_id, user_id, badge_id)`
...
1. 정산 전용 WAS 단일 리더 선점
2. `PENDING` 대상 소량 조회
3. 짧은 Tx로 UPDATE → `APPLIED`
4. 실패는 `FAILED`로 기록, 재시도 가능

### 장애·복구

* `APPLIED`는 유지, `PENDING/FAILED`만 재처리
* 자동 재개 트리거는 미구현, 필요 시 `executionId` 단위로 재실행 가능

---

## 전통 로그 테이블 vs 경량 로그 테이블

| 구분     | 전통 로그 테이블               | 경량 로그 테이블       |
| ------ | ----------------------- | --------------- |
| 원본 테이블 | 최신/과거 섞여 있음             | 항상 최신 유지        |
| 로그 테이블 | 모든 변경 내역 기록             | 정산 중 최초 변경만 기록  |
| 정산 조회  | `MAX(timestamp)` 필요     | 단순 JOIN + 조건 분기 |
| 쿼리 비용  | 높음 (윈도우 함수, 정렬, 그룹화 필요) | 낮음 (조건 분기만 수행)  |
| 장점     | 범용성, 과거 시점 복원 가능        | 성능 최적화, 단순 쿼리   |
| 단점     | 대규모 정산 시 부하 큼           | 범용성 부족          |

---

## 4.5 결론

* 전통 로그 테이블 대비 쿼리 비용이 낮아 성능에 최적화되었다.
* 범용성은 부족하지만, 본 프로젝트 목표(성능 + 정합성 확보)에는 적합하다.
* 따라서 **정산 배치 최적화 구조로 합리적인 선택**이었다.
